...
const AdicionarCredores: React.FC<AddCredoresProps> = ({ open, setOpen }) => {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        setLoading(true);
        try {
            const response = await axiosInstance.post("/credores", data);
            if (response.status === 201) {
                toast({
                    title: "Sucesso ao adicionar o credor!",
                    description: "Credor adicionado com sucesso.",
                })
                refetch();
                setOpen(false);
                form.reset();
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent className="sm:max-w-[50%]">
                <DialogHeader>
                    <h1 className="font-bold text-lg">Adicionar Credor</h1>
                </DialogHeader>
                <Form {...form}>
...
export default AdicionarCredores;