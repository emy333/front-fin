...
const EditarCredor: React.FC<EditaCredorProps> = ({ open, setOpen, idCredor }) => {
    ...
    useEffect(() => {
        if (credor) {
            form.setValue("descricao", credor.descricao);
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        ...
        try {
            const response = await axiosInstance.put(`/credores/${idCredor}`, dados);
            if (response.status === 200) {
                toast({
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent className="sm:max-w-[50%]">
                <DialogHeader>
                    <h1 className="font-bold text-lg">Visualizar Credor</h1>
                </DialogHeader>
