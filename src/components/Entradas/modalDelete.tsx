...
export const DeleteEntrada: React.FC<Props> = ({ open, setOpen, idClicked, refetch }) => {
    const handleDelete = async () => {
        try {
            const response = await axiosInstance.delete(`/entradas/${idClicked}`);
        ...
        <AlertDialog open={open} onOpenChange={setOpen}>
            <AlertDialogContent className="max-w-md p-6 rounded-lg shadow-lg bg-white">
                <AlertDialogHeader className="flex flex-col items-center">
                    ...
                    <AlertDialogTitle className="text-lg font-semibold text-gray-800 dark:text-white mt-4">Tem certeza que deseja excluir a entrada?<br />
                    </AlertDialogTitle>
