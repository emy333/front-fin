...
const GatosFixosParcelados: React.FC<dataProps> = ({ periodo }) => {
  ...
  const { data: totalGastosFixosParc, isLoading: isLoadingTot } = useGetTotFixosParcelados(periodo, Number(id_usuario));
    ...
    <Card className="col-span-3">
      <CardHeader className="h-20">
        <div className="flex justify-between mb-1">
          <CardTitle className="text-xl text-foreground">Saídas Fixas e Parceladas</CardTitle>
          <p className="text-lg text-foreground font-bold">
            {isLoadingTot ? <Skeleton className="h-6 w-24" /> : formatCurrency(totalGastosFixosParc)}
          </p>
        </div>

        <div className="flex gap-2">
          <div className="flex items-center gap-1 text-sm"><div className="bg-green-800 dark:bg-green-600 h-5 w-5 rounded-sm "></div> PAGO</div>
          <div className="flex items-center gap-1 text-sm"><div className="bg-slate-800 dark:bg-slate-600 h-5 w-5 rounded-sm"></div> NÃO PAGO</div>
        </div>
      </CardHeader>
                ...
                >
                  <div className="flex flex-col space-y-1 w-full sm:w-2/3">
                    <div className="flex gap-2 items-center">
                      <p className="text-lg font-medium truncate">{gasto.descricao.toUpperCase()}</p>
                    </div>
